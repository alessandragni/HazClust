#' Compute Distance Matrix for Penalized Likelihood in Parametric Frailty Models
#'
...
#' @param Fmatr Numeric matrix used as the penalty component in the optimization, related to cluster structure.
#' @param lambda Numeric penalty weight scalar for the \code{Fmatr} matrix.
...
#'
#' @details
#' The function computes a composite distance matrix that integrates:
#' \itemize{
#'   \item Squared Euclidean distances of covariate values within clusters,
#'   \item Penalized differences derived from frailty-adjusted linear predictors and cumulative hazards,
#'   \item An optional penalty term scaled by \code{lambda} applied to the matrix \code{Fmatr}.
#' }
#' These components regularize the estimation of frailty model parameters for clustered survival data.
#'
...
#' @keywords internal
computeD <- function(formula, cluster, strata, data, dist,
                     frailty, p,
                     Fmatr, # matrix F
                     lambda, # lambda
                     transform = TRUE,
  ...
  if (frailty != "none")  {
    ...
    Xbeta_i <- Xbeta[,1] 
    Xonly_i <- do.call(rbind, lapply(levels(as.factor(obs$strata)), function(x) {
    ...
    # Xonly_i <- do.call(rbind, apply(cbind(rownames(pars), pars), 1,
    #                           function(x) {
    #                             as.matrix(obs$x[
    #                               obs$strata == x[1], -1, drop = FALSE])
    #                           }))
    Xonly_i <- do.call(rbind, lapply(1:nrow(pars), function(i) {
      stratum <- rownames(pars)[i]
      as.matrix(obs$x[obs$strata == stratum, -1, drop = FALSE])
    }))
