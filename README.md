...
- `TabS62_2StepAnalysisAccuracyARI.ipynb` reproduces  the results reported in Table S6.2 (referenced in Section S6.1). 
   This script relies on alternative_simulation.job, alternative.R, and enhanced_simulate_data.R.
   The outputs are stored in the `output/tabS62` folder.
   
   To generate the results, run:
   ```bash
   chmod +x submit_TabS62_simulations.sh
   ./submit_TabS62_simulations.sh
   ```
   
   This notebook relies on the outputs stored in the `output/tab2` folder.  
   
- `TabS63_Figs_Variabandestimation.ipynb` reproduces 
   - the descriptive results in Section 3.1.1
   - Table S6.3 (referenced in Web Supplementary Materials), 
